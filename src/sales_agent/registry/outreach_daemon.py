...
Can also be run standalone:
    uv run python src/sales_agent/registry/outreach_daemon.py
"""
...
from sales_agent.registry.models import ProspectStatus
...
class OutreachDaemon:
    ...
    async def _process_assignments(self) -> None:
        ...
        for prospect in unassigned:
            ...
            # Assign prospect
            success = await prospect_manager.assign_prospect_to_rep(prospect.id, rep.id)
            if success:
    ...
    async def _get_next_rep_with_capacity(self, active_reps: list) -> Optional[object]:
        ...
        for _ in range(len(active_reps)):
            ...
            # Check capacity
            current_count = await prospect_manager.count_prospects_for_rep(rep.id)
