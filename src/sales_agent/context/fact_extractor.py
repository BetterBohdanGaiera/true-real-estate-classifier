    ...
    def format_for_context(self) -> str:
        ...
        if self.budget_min is not None or self.budget_max is not None:
            if self.budget_min == self.budget_max and self.budget_min is not None:
                parts.append(f"- Бюджет: {self.budget_min:,} {self.budget_currency}")
            elif self.budget_min is not None and self.budget_max is not None:
                parts.append(f"- Бюджет: {self.budget_min:,}-{self.budget_max:,} {self.budget_currency}")
            elif self.budget_min is not None:
                parts.append(f"- Бюджет: от {self.budget_min:,} {self.budget_currency}")
            else:
                parts.append(f"- Бюджет: до {self.budget_max:,} {self.budget_currency}")
        ...
        if self.property_type:
            ...
            parts.append(f"- Тип объекта: {type_labels.get(self.property_type, self.property_type)}")

        if self.location_preferences:
            parts.append(f"- Локации: {', '.join(self.location_preferences)}")

        if self.purpose:
            purpose_labels = {
                "investment": "инвестиции",
                "living": "для проживания",
                "rental": "для сдачи в аренду",
            }
            parts.append(f"- Цель: {purpose_labels.get(self.purpose, self.purpose)}")

        if self.bedrooms:
            parts.append(f"- Спален: {self.bedrooms}")

        if self.features:
            parts.append(f"- Требования: {', '.join(self.features)}")

        if self.timeline:
            parts.append(f"- Сроки: {self.timeline}")
        ...
        return "Известные факты о клиенте:\n" + "\n".join(parts)
...
class FactExtractor:
    ...
    Example:
        ...
        >>> print(facts.location_preferences)  # ["чангу"]
    """

    # Budget patterns - capture numeric amounts with various formats
    BUDGET_PATTERNS = [
        (r"(\d+)\s*[kкК]", 1000),           # 500k, 500к
        (r"\$\s*(\d[\d\s]*\d)", 1),          # $500000, $ 500 000
        (r"(\d[\d\s]*\d)\s*\$", 1),          # 500000$
        (r"(\d+)\s*тыс", 1000),              # 500 тысяч
        (r"(\d+(?:[.,]\d+)?)\s*млн", 1000000),  # 1 млн, 1.5 млн
        (r"бюджет[:\s]+(\d[\d\s]*)", 1),    # бюджет: 500000
        (r"(\d[\d\s]*\d)\s*(?:долл|usd)", 1),  # 500000 долларов/usd
    ]

    # Budget range patterns - capture min-max ranges
    BUDGET_RANGE_PATTERNS = [
        (r"(\d+)\s*[-–]\s*(\d+)\s*[kкК]", 1000),   # 300-500k, 300-500к
        (r"от\s*(\d+)\s*до\s*(\d+)\s*[kкК]", 1000),  # от 300 до 500к
    ...
    # Property type patterns - Russian regex patterns mapped to types
    PROPERTY_TYPES = {
        r"вилл[ауыей]": "villa",
        r"апартамент[ыа]?": "apartment",
        r"земл[яюией]|участ[ок]": "land",
        r"дом[а]?": "house",
        r"студи[яюией]": "studio",
        r"таунхаус": "townhouse",
        r"пентхаус": "penthouse",
    }

    # Purpose indicators
    PURPOSE_PATTERNS = {
        "investment": [r"инвестиц", r"вложен", r"доход", r"roi", r"окупаем"],
        "living": [r"жить", r"переезд", r"переехать", r"пмж", r"постоянн"],
        "rental": [r"сдавать", r"аренд", r"арендн", r"rental"],
    }

    # Timeline patterns
    TIMELINE_PATTERNS = [
        (r"срочно|асап|asap|как можно скорее", "urgent"),
        (r"в ближайш|скоро|на днях", "within_month"),
        (r"(\d+)\s*мес", "{} months"),
        (r"через\s*год|в следующем году", "1 year"),
        (r"не срочно|не торопл", "no_rush"),
    ]

    # Bedroom patterns
    BEDROOM_PATTERNS = [
        r"(\d+)\s*(?:спальн|bedroom|br|бр)",
        r"(\d+)\s*(?:комнат)",
    ]

    # Feature patterns
    FEATURE_KEYWORDS = {
        "pool": [r"бассейн", r"pool"],
        "ocean_view": [r"вид на океан", r"ocean view", r"с видом"],
        "rice_field_view": [r"вид на рис", r"rice field", r"рисов"],
        "private": [r"приватн", r"private", r"уедин"],
        "gated": [r"охраняем", r"gated", r"закрыт"],
        "new": [r"нов[ыа]", r"new build", r"свеж"],
        "furnished": [r"мебел", r"furnished", r"обставлен"],
    }
    ...
    def _extract_budget(self, message: str, facts: ExtractedFacts) -> None:
        ...
        for pattern, multiplier in self.BUDGET_PATTERNS:
            ...
            if match:
                ...
                try:
                    # Handle decimal amounts (e.g., 1.5 млн)
    ...
    def _extract_locations(self, message: str, facts: ExtractedFacts) -> None:
        ...
        for loc in self.LOCATION_KEYWORDS:
            if loc in message and loc not in facts.location_preferences:
                # Normalize to Russian version if English
                normalized = loc
                # Simple normalization - map English to Russian
                loc_map = {
                    "canggu": "чангу",
                    "seminyak": "семиньяк",
                    "ubud": "убуд",
                ...
                normalized = loc_map.get(loc, loc)
