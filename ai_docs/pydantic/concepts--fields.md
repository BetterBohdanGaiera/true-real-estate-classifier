...
[`pydantic.fields.Field`](../../api/fields/#pydantic.fields.Field)
...
To do so, the [`Field()`](../../api/fields/#pydantic.fields.Field) function is used a lot, and behaves the same way as
...
to emphasize on the fact that a value must be provided, you can use the [ellipsis](https://docs.python.org/3/library/constants.html#Ellipsis):
...
## The annotated pattern[¶](#the-annotated-pattern "Permanent link")

To apply constraints or attach [`Field()`](../../api/fields/#pydantic.fields.Field) functions to a model field, Pydantic
...
* You can provide an arbitrary amount of metadata elements for a field. As shown in the example above,
  the [`Field()`](../../api/fields/#pydantic.fields.Field) function only supports a limited set of constraints/metadata,
  and you may have to use different Pydantic utilities such as [`WithJsonSchema`](../../api/json_schema/#pydantic.json_schema.WithJsonSchema)
  in some cases.
* Types can be made reusable (see the documentation on [custom types](../types/#using-the-annotated-pattern)
  using this pattern).

However, note that certain arguments to the [`Field()`](../../api/fields/#pydantic.fields.Field) function (namely, `default`,
...
1. The [`Field()`](../../api/fields/#pydantic.fields.Field) function is applied to `int` type, hence the
   `deprecated` flag won't have any effect. While this may be confusing given that the name of
   the [`Field()`](../../api/fields/#pydantic.fields.Field) function would imply it should apply to the field,
...
2. The [`Field()`](../../api/fields/#pydantic.fields.Field) function is applied to the "top-level" union type,
   hence the `deprecated` flag will be applied to the field.

## Inspecting model fields[¶](#inspecting-model-fields "Permanent link")

The fields of a model can be inspected using the [`model_fields`](../../api/base_model/#pydantic.BaseModel.model_fields) class attribute
(or the `__pydantic_fields__` attribute for [Pydantic dataclasses](../dataclasses/)). It is a mapping of field names
to their definition (represented as [`FieldInfo`](../../api/fields/#pydantic.fields.FieldInfo) instances).
...
[In Pydantic V1](../../migration/#required-optional-and-nullable-fields), a type annotated as [`Any`](https://docs.python.org/3/library/typing.html#typing.Any)
...
## Validate default values[¶](#validate-default-values "Permanent link")

By default, Pydantic will *not* validate default values. The `validate_default` field parameter
(or the [`validate_default`](../../api/config/#pydantic.config.ConfigDict.validate_default) configuration value) can be used
...
### Mutable default values[¶](#mutable-default-values "Permanent link")
...
## Field aliases[¶](#field-aliases "Permanent link")
...
2. We are using [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump) to convert the model into a serializable format.
   ...
   You can also use [`ConfigDict.serialize_by_alias`](../../api/config/#pydantic.config.ConfigDict.serialize_by_alias) to
...
If you provide a value for the [`alias_generator`](../../api/config/#pydantic.config.ConfigDict.alias_generator) model setting, you can control the order of precedence for field alias and generated aliases via the `alias_priority` field parameter. You can read more about alias precedence [here](../alias/#alias-precedence).
...
This means that when using the [`validate_by_name`](../../api/config/#pydantic.config.ConfigDict.validate_by_name) model setting (which allows both the field name and alias to be used during model validation), type checkers will error when the actual field name is used:
...
## Field constraints[¶](#field-constraints "Permanent link")

The [`Field()`](../../api/fields/#pydantic.fields.Field) function can also be used to add constraints to specific types:
...
in the [standard library types](../../api/standard_library_types/) documentation.

## Strict fields[¶](#strict-fields "Permanent link")

The `strict` parameter of the [`Field()`](../../api/fields/#pydantic.fields.Field) function specifies whether the field should be validated in
...
## Dataclass fields[¶](#dataclass-fields "Permanent link")

Some parameters of the [`Field()`](../../api/fields/#pydantic.fields.Field) function can be used on [dataclasses](../dataclasses/):
...
## Field Representation[¶](#field-representation "Permanent link")
...
## Discriminator[¶](#discriminator "Permanent link")
...
## Immutability[¶](#immutability "Permanent link")
...
## Excluding fields[¶](#excluding-fields "Permanent link")
...
1. The `age` field is not included in the [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump) output, since it is excluded.

See the dedicated [serialization section](../serialization/#field-inclusion-and-exclusion) for more details.

## Deprecated fields[¶](#deprecated-fields "Permanent link")
...
### `deprecated` as a string[¶](#deprecated-as-a-string "Permanent link")
...
### `deprecated` via the `@warnings.deprecated` decorator[¶](#deprecated-via-the-warningsdeprecated-decorator "Permanent link")
...
### `deprecated` as a boolean[¶](#deprecated-as-a-boolean "Permanent link")
...
## Customizing JSON Schema[¶](#customizing-json-schema "Permanent link")
...
## The `computed_field` decorator[¶](#the-computed_field-decorator "Permanent link")

API Documentation

[`computed_field`](../../api/fields/#pydantic.fields.computed_field)

The [`computed_field`](../../api/fields/#pydantic.fields.computed_field) decorator can be used to include [`property`](https://docs.python.org/3/library/functions.html#property) or
...
1. If not specified, [`computed_field`](../../api/fields/#pydantic.fields.computed_field) will implicitly convert the method
