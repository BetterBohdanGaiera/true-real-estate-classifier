...
[`pydantic.main.BaseModel`](../../api/base_model/#pydantic.BaseModel)

One of the primary ways of defining schema in Pydantic is via models. Models are simply classes which inherit from
[`BaseModel`](../../api/base_model/#pydantic.BaseModel) and define fields as annotated attributes.
...
## Basic model usage[¶](#basic-model-usage "Permanent link")
...
1. Pydantic models support a variety of [configuration values](../config/)
   (see [here](../../api/config/#pydantic.config.ConfigDict) for the available configuration values).
...
Fields can be customized in a number of ways using the [`Field()`](../../api/fields/#pydantic.fields.Field) function.
...
If no [`ValidationError`](../../api/pydantic_core/#pydantic_core.ValidationError) exception is raised,
...
1. `name` wasn't set when `user` was initialized, so the default value was used.
   The [`model_fields_set`](../../api/base_model/#pydantic.BaseModel.model_fields_set) attribute can be
...
The model instance can be serialized using the [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump) method:
...
recursively converted into dictionaries. [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump) also
...
### Model methods and properties[¶](#model-methods-and-properties "Permanent link")
...
* [`model_validate()`](../../api/base_model/#pydantic.BaseModel.model_validate): Validates the given object against the Pydantic model. See [Validating data](#validating-data).
* [`model_validate_json()`](../../api/base_model/#pydantic.BaseModel.model_validate_json): Validates the given JSON data against the Pydantic model. See
  [Validating data](#validating-data).
* [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct): Creates models without running validation. See
  [Creating models without validation](#creating-models-without-validation).
* [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump): Returns a dictionary of the model's fields and values. See
  [Serialization](../serialization/#python-mode).
* [`model_dump_json()`](../../api/base_model/#pydantic.BaseModel.model_dump_json): Returns a JSON string representation of [`model_dump()`](../../api/base_model/#pydantic.BaseModel.model_dump). See [Serialization](../serialization/#json-mode).
* [`model_copy()`](../../api/base_model/#pydantic.BaseModel.model_copy): Returns a copy (by default, shallow copy) of the model. See
  [Model copy](#model-copy).
* [`model_json_schema()`](../../api/base_model/#pydantic.BaseModel.model_json_schema): Returns a jsonable dictionary representing the model's JSON Schema. See [JSON Schema](../json_schema/).
* [`model_fields`](../../api/base_model/#pydantic.BaseModel.model_fields): A mapping between field names and their definitions ([`FieldInfo`](../../api/fields/#pydantic.fields.FieldInfo) instances).
* [`model_computed_fields`](../../api/base_model/#pydantic.BaseModel.model_computed_fields): A mapping between computed field names and their definitions ([`ComputedFieldInfo`](../../api/fields/#pydantic.fields.ComputedFieldInfo) instances).
* [`model_parametrized_name()`](../../api/base_model/#pydantic.BaseModel.model_parametrized_name): Computes the class name for parametrizations of generic classes.
* [`model_post_init()`](../../api/base_model/#pydantic.BaseModel.model_post_init): Performs additional actions after the model is instantiated and all field validators are applied.
* [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild): Rebuilds the model schema, which also supports building recursive generic models.
  See [Rebuilding model schema](#rebuilding-model-schema).

Model instances possess the following attributes:

* [`model_extra`](../../api/base_model/#pydantic.BaseModel.model_extra): The extra fields set during validation.
* [`model_fields_set`](../../api/base_model/#pydantic.BaseModel.model_fields_set): The set of fields which were explicitly provided when the model was initialized.

Note

See the API documentation of [`BaseModel`](../../api/base_model/#pydantic.BaseModel) for the class definition including a full list of methods and attributes.
...
## Extra data[¶](#extra-data "Permanent link")
...
The [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) configuration value can be used to control this behavior:
...
1. If [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) was set to `'forbid'`, this would fail.
...
The validation methods (e.g. [`model_validate()`](../../api/base_model/#pydantic.BaseModel.model_validate)) have an optional `extra` argument
that will override the `extra` configuration value of the model for that validation call.

For more details, refer to the [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) API documentation.
...
## Rebuilding model schema[¶](#rebuilding-model-schema "Permanent link")
...
To circumvent this issue, the [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild) method can be used:
...
call [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild) proactively when dealing with recursive models or generics.

In V2, [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild) replaced `update_forward_refs()` from V1. There are some slight differences with the new behavior.
The biggest change is that when calling [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild) on the outermost model, it builds a core schema used for validation of the
whole model (nested models and all), so all types at all levels need to be ready before [`model_rebuild()`](../../api/base_model/#pydantic.BaseModel.model_rebuild) is called.

## Validating data[¶](#validating-data "Permanent link")
...
* [`model_validate()`](../../api/base_model/#pydantic.BaseModel.model_validate): data can be provided either as a dictionary,
  or as a model instance (by default, instances are assumed to be valid; see the [`revalidate_instances`](../../api/config/#pydantic.config.ConfigDict.revalidate_instances)
...
* [`model_validate_json()`](../../api/base_model/#pydantic.BaseModel.model_validate_json): data is validated as a JSON string or `bytes` object.
  ...
  Learn more about JSON parsing in the [JSON](../json/) documentation.
* [`model_validate_strings()`](../../api/base_model/#pydantic.BaseModel.model_validate_strings): data is validated as a dictionary (can be nested) with
...
([strictness](../strict_mode/), [extra data](#extra-data), [validation context](../validators/#validation-context), etc.).

Note

Depending on the types and model configuration involved, the *Python* and *JSON* modes may have different validation behavior (e.g. with [strictness](../strict_mode/)).
...
either dump your data to JSON (e.g. using [`json.dumps()`](https://docs.python.org/3/library/json.html#json.dumps)), or use [`model_validate_strings()`](../../api/base_model/#pydantic.BaseModel.model_validate_strings)
...
### Creating models without validation[¶](#creating-models-without-validation "Permanent link")

Pydantic also provides the [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct) method, which allows models to be created **without validation**.
...
[`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct) does not do any validation, meaning it can create
models which are invalid. **You should only ever use the [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct)
...
and [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct) has been narrowed
considerably. For simple models, going with validation may even be faster. If you are using [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct)
...
[`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct) positionally, instead of using a keyword argument.

Here are some additional notes on the behavior of [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct):
...
On [extra data](#extra-data) behavior with [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct)

* For models with [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) set to `'allow'`, data not corresponding to fields will be correctly stored in
  the `__pydantic_extra__` dictionary and saved to the model's `__dict__` attribute.
* For models with [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) set to `'ignore'`, data not corresponding to fields will be ignored — that is,
  not stored in `__pydantic_extra__` or `__dict__` on the instance.
* Unlike when instantiating the model with validation, a call to [`model_construct()`](../../api/base_model/#pydantic.BaseModel.model_construct) with [`extra`](../../api/config/#pydantic.config.ConfigDict.extra) set to `'forbid'` doesn't raise an error in the presence of data not corresponding to fields. Rather, said input data is simply ignored.

### Defining a custom `__init__()`[¶](#defining-a-custom-__init__ "Permanent link")
...
Defining a custom `__init__()` is not recommended, as all the validation parameters ([strictness](../strict_mode/),
[extra data behavior](#extra-data), [validation context](../validators/#validation-context)) will be lost. If you need to perform
actions after the model was initialized, you can make use of *after* [field](../validators/#field-after-validator) or
[model](../validators/#model-after-validator) validators, or define a [`model_post_init()`](../../api/base_model/#pydantic.BaseModel.model_post_init) implementation:
...
Pydantic will raise a [`ValidationError`](../../api/pydantic_core/#pydantic_core.ValidationError) exception whenever it finds an error in the data it's validating.
...
## Arbitrary class instances[¶](#arbitrary-class-instances "Permanent link")

(Formerly known as "ORM Mode"/`from_orm()`).

When using the [`model_validate()`](../../api/base_model/#pydantic.BaseModel.model_validate) method, Pydantic can also validate arbitrary objects,
...
This feature need to be manually enabled, either by setting the [`from_attributes`](../../api/config/#pydantic.config.ConfigDict.from_attributes)
configuration value, or by using the `from_attributes` parameter on [`model_validate()`](../../api/base_model/#pydantic.BaseModel.model_validate).
...
### Nested attributes[¶](#nested-attributes "Permanent link")
...
## Model copy[¶](#model-copy "Permanent link")

API Documentation

[`pydantic.main.BaseModel.model_copy`](../../api/base_model/#pydantic.BaseModel.model_copy)

The [`model_copy()`](../../api/base_model/#pydantic.BaseModel.model_copy) method allows models to be duplicated (with optional updates),
...
## Generic models[¶](#generic-models "Permanent link")
...
2. Declare a Pydantic model that inherits from [`BaseModel`](../../api/base_model/#pydantic.BaseModel) and [`typing.Generic`](https://docs.python.org/3/library/typing.html#typing.Generic)
...
by overriding the [`model_parametrized_name()`](../../api/base_model/#pydantic.BaseModel.model_parametrized_name) method:
...
### Validation of unparametrized type variables[¶](#validation-of-unparametrized-type-variables "Permanent link")
...
### Serialization of unparametrized type variables[¶](#serialization-of-unparametrized-type-variables "Permanent link")
...
## Dynamic model creation[¶](#dynamic-model-creation "Permanent link")

API Documentation

[`pydantic.main.create_model`](../../api/base_model/#pydantic.create_model)

There are some occasions where it is desirable to create a model using runtime information to specify the fields.
Pydantic provides the [`create_model()`](../../api/base_model/#pydantic.create_model) function to allow models to be created dynamically:
...
* A two-tuple, the first element being the type and the second element the assigned value
  (either a default or the [`Field()`](../../api/fields/#pydantic.fields.Field) function).
...
## `RootModel` and custom root types[¶](#rootmodel-and-custom-root-types "Permanent link")

API Documentation

[`pydantic.root_model.RootModel`](../../api/root_model/#pydantic.root_model.RootModel)

Pydantic models can be defined with a "custom root type" by subclassing [`pydantic.RootModel`](../../api/root_model/#pydantic.root_model.RootModel).

The root type can be any type supported by Pydantic, and is specified by the generic parameter to `RootModel`.
The root value can be passed to the model `__init__` or [`model_validate`](../../api/base_model/#pydantic.BaseModel.model_validate)
...
## Faux immutability[¶](#faux-immutability "Permanent link")

Models can be configured to be immutable via `model_config['frozen'] = True`. When this is set, attempting to change the
values of instance attributes will raise errors. See the [API reference](../../api/config/#pydantic.config.ConfigDict.frozen) for more details.
...
## Abstract base classes[¶](#abstract-base-classes "Permanent link")
...
## Field ordering[¶](#field-ordering "Permanent link")
...
## Automatically excluded attributes[¶](#automatically-excluded-attributes "Permanent link")

### Class variables[¶](#class-variables "Permanent link")
...
### Private model attributes[¶](#private-model-attributes "Permanent link")

API Documentation

[`pydantic.fields.PrivateAttr`](../../api/fields/#pydantic.fields.PrivateAttr)
...
## Model signature[¶](#model-signature "Permanent link")
...
## Structural pattern matching[¶](#structural-pattern-matching "Permanent link")
...
## Attribute copies[¶](#attribute-copies "Permanent link")
