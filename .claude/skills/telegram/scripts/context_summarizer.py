...
class ContextSummarizer:
    ...
    def build_context_with_summary(
        ...
        if summary:
            parts.append(f"[Kratkoye soderzhaniye predydushchego razgovora]\n{summary}\n")

        if recent_messages:
            parts.append("[Posledniye soobshcheniya]")
            ...
            for msg in messages_to_include:
                ...
                if hasattr(msg, 'sender') and hasattr(msg, 'text'):
                    sender = "Vy" if msg.sender == "agent" else "Kliyent"
                    text = msg.text
                    ...
                    sender = "Vy" if msg.get('sender') == "agent" else "Kliyent"
                    text = msg.get('text', '')
                    parts.append(f"{sender}: {text}")
                else:
